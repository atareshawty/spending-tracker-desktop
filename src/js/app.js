...
window.App = (function() {
  ...
  return {
    ...
    buildTable: function(filteredSpending, filteredTotal) {
      $.get('./src/views/templates/spending_table.handlebars', function(source) {
        var spending = filteredSpending || user.spending;
        var total = filteredTotal || user.formattedTotalSpending;
        if (spending.length > 0) {
          var newHTML = Handlebars.compile(source)({spending: spending, total: total});
    ...
    buildIncomeTable: function(filteredIncome, filteredTotal) {
      $.get('./src/views/templates/income_table.handlebars', function(source) {
        var income = filteredIncome || user.income;
        var total = filteredTotal || user.formattedTotalIncome;
        if (income.length > 0) {
          var newHTML = Handlebars.compile(source)({income: income, total: total});
